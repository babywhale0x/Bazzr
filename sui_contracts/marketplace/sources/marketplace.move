module marketplace::verixa {
    ...
    const VIEW_DURATION_MS: u64 = 86400 * 1000;        // 24 hours
    const BORROW_DURATION_MS: u64 = 604800 * 1000;      // 7 days
    ...
    fun get_tier_details(content: &Content, tier: u8, current_time: u64): (u64, u64) {
        ...
        let expiry = if (tier == TIER_VIEW) {
            current_time + VIEW_DURATION_MS
        } else if (tier == TIER_BORROW) {
            current_time + BORROW_DURATION_MS
        } else {
            0 // Permanent
        };
    ...
    // Since Sui uses on-chain query through RPC or graphql for object states, we don't strictly need pure getter functions,
    // but the frontend can query the MarketplaceState object and inspect the dynamic fields / tables.
}
